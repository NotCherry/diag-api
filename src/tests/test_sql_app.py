...
def test_create_user_project():
    ...
    response = client.post("/project", json=TEST_PROJECT, headers=auth_header())
...
def test_create_diagram_in_project():
    response = client.post(
        f"/project/{1}/diagram", json=TEST_DIAGRAM, headers=auth_header()
    )
...
def test_create_org():
    response = client.post(
        f"/organization", json=TEST_ORGANIZATION, headers=auth_header()
    )
...
def test_create_org_project():
    for x in range(2):
        post_data = {"org_id": 1, "project": TEST_PROJECT}

        response = client.post(
            f"/organization/project", json=post_data, headers=auth_header()
        )
